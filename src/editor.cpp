    ...
    switch (current_mode) {
        ...
        case 1: {
            ...
            if (game->num_point_lights > 0) {
                ...
                ImGui::DragFloat("radius", &light->radius, 0.005f, 0.0f, F32_MAX, "%.3f");
                
                immediate_begin();
                set_texture(0);
                // Circle billboard to show radius.
                immediate_cube(light->position, 0.2f, v4(light->color, 1.0f));
                immediate_torus(light->position, light->radius, normalize0(light->position - game->camera.position), v4(1), 0.01f);
                immediate_end();
            }
        } break;
        case 2: {
